var listaEmails = [];
        ...
        function limparTodos() {
            listaEmails = []; // Limpa a lista de e-mails
            localStorage.removeItem('emails'); // Remove os e-mails salvos no localStorage
            atualizarResultados(); // Atualiza a exibição na página
            atualizarPaginacao(); // Atualiza a paginação (botões de página)
        }

        // Função para buscar e-mails
        function buscarEmail() {
            var termoBusca = document.getElementById('busca').value.toLowerCase();
            var resultadosFiltrados = listaEmails.filter(function(item) {
                return item.email.toLowerCase().includes(termoBusca);
            });
    ...
    reader.onload = function(e) {
        ...
        lines.forEach(function(line) {
            ...
            var email = columns[0].trim(); // Correção para pegar o e-mail corretamente
            var senha = columns[1] ? columns[1].trim() : ''; // Verifica se há senha na segunda coluna
